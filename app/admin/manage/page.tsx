    ...
    <Screen me={me} wide>
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="table" style={{ fontSize: 13, minWidth: 560 }}>
            <thead><tr><th>Name</th><th>Role</th><th>City</th><th className="ta-r">Base salary</th><th className="ta-r">Daily min</th><th>Line</th><th></th></tr></thead>
            <tbody>
              {data.users.map((u: any) => (
                <tr key={u.id}>
                  ...
                  <td className="ta-r">
                    ...
                    {canEdit ? <button className="btn btn-ghost fs-caption" onClick={() => setEditUser({ ...u, baseSalary: groupDigits(String(u.baseSalary ?? "")) })}>Edit</button> : null}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {editUser ? (
          <div className="card gap-3">
            ...
            <div className="two-col gap-3">
              <div className="field m0"><label>Name</label><input className="input" value={editUser.name} onChange={(e) => setEditUser({ ...editUser, name: e.target.value })} /></div>
              <div className="field m0"><label>Phone</label><input className="input" value={editUser.phone} onChange={(e) => setEditUser({ ...editUser, phone: e.target.value })} /></div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10 }}>
              <div className="field m0"><label>Role</label>
              ...
              <div className="field m0"><label>Base salary</label><input className="input" inputMode="numeric" value={editUser.baseSalary} onChange={(e) => setEditUser({ ...editUser, baseSalary: groupDigits(e.target.value) })} /></div>
              <div className="field m0"><label>Daily min</label><input className="input" inputMode="numeric" value={editUser.dailyMin} onChange={(e) => setEditUser({ ...editUser, dailyMin: e.target.value.replace(/[^0-9]/g, "") })} /></div>
        ...
        {canEdit ? (
          <div className="card gap-3">
            <div className="row">
              <h6 className="m0 f1">Hand a territory over</h6>
...
Continue anyway?`)) return;
        ...
        <table className="table fs-small">
          <thead><tr><th>Product</th><th>SKU</th><th className="ta-r">Unit price</th><th></th></tr></thead>
          <tbody>
            {data.products.map((p: any) => (
              <tr key={p.id}>
                ...
                <td className="ta-r">
                  ...
                  {canEdit ? <button className="btn btn-ghost fs-caption" onClick={() => setEditProduct({ ...p, unitPrice: groupDigits(String(p.unitPrice ?? "")) })}>Edit</button> : null}
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {editProduct ? (
          <div className="card gap-3">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10 }}>
              <div className="field m0"><label>Name</label><input className="input" value={editProduct.name} onChange={(e) => setEditProduct({ ...editProduct, name: e.target.value })} /></div>
              ...
              <div className="field m0"><label>Price (IQD)</label><input className="input" inputMode="numeric" value={editProduct.unitPrice} onChange={(e) => setEditProduct({ ...editProduct, unitPrice: groupDigits(e.target.value) })} /></div>
              <div className="field m0"><label>Unit</label><input className="input" value={editProduct.unit} onChange={(e) => setEditProduct({ ...editProduct, unit: e.target.value })} /></div>
              <div className="field m0">
                <label>Line</label>
